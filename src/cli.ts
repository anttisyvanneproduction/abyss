...
const defaultMemoryPath = ".abyss/memory.jsonl";
...
program
  ...
  .option("--harnesses <list>", "comma-separated agent CLI IDs (default: all detected)")
  ...
  .option("--launch", "auto-launch agent CLIs that support headless mode (claude -p, codex -q)")
  ...
  .action(async (taskParts: string[], options: {
    ...
    if (combatants.length < 2) {
      console.error(combatants.length === 0
        ? "No agent CLIs detected. Install Claude Code, Codex, or GitHub Copilot, or specify --harnesses."
...
program
  .command("setup")
  .description("configure Abyss Stack for each selected agent CLI (skills, MCP, hooks, instruction files)")
  .option("--force", "overwrite existing files and entries")
  .addHelpText("after", `
Examples:
  abyss setup               interactive agent CLI + skill profile selection
...
program
  .command("uninstall")
  .description("remove Abyss Stack configuration for selected agent CLIs")
  .action(async () => {
    await runUninstall({});
  });
...
Examples:
  ...
  .action(async (skills: string[], options: { list?: boolean; all?: boolean; allall?: boolean; opex?: boolean; category?: string; path: string; agentsPath: string; spice?: string; force?: boolean; yes?: boolean }) => {
    ...
    if (harnesses.length === 0) {
      console.log("No agent CLI selected. Run 'abyss setup' later to wire the routing index.");
      return;
    }
...
function getImgflipConfigPath(): string {
  return join(homedir(), ".abyss", "imgflip.json");
}
