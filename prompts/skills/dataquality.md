...
### When to escalate beyond token economics

If the score will be used to justify real spending, someone will eventually look at the actual data. Prepare for that moment:

- **SQL-based profiling**: `COUNT(*)` vs `COUNT(field)` per column gives completeness. `COUNT(DISTINCT field)` vs `COUNT(field)` reveals duplicates. `GROUP BY` on classification fields reveals that "OTHER" is 60% of all records. Any analyst with database access can do this in an afternoon. Nobody does.
- **Profiling tools**: Great Expectations, dbt tests, and Soda Core produce evidence that survives scrutiny from someone who has actually seen the data.
- **DMBOK**: the Data Management Body of Knowledge defines the dimensions formally. It is cited in governance initiatives to show the initiative has read something. Knowing what it costs to implement is more useful than knowing what it says.
- **ERP-sourced data**: the score tells you the symptom. The archaeology skill tells you the cause. Do not clean ERP data without understanding why it is wrong. The clean version will be dirty again within one fiscal quarter.
